...
!
    ...
    CALL CAL_DUSTL23_EmisFlux(HcoState, ExtState, Inst, FLUX, RC)
    ! Error check
    IF ( RC /= HCO_SUCCESS ) THEN
      ERR = .TRUE.
      RETURN
...
!
    ...
    ! Get species IDs.
    CALL HCO_GetExtHcoID( HcoState, ExtNr, Inst%HcoIDs, Inst%SpcNames, Inst%nSpc, RC )
    IF ( RC /= HCO_SUCCESS ) THEN
...
!BOC
  ...
  SUBROUTINE CAL_THR_FRIC_VEL(HcoState, rho_a, f_clay, f_sand, theta, &
    ...
    DO J = 1, HcoState%NY
        DO I = 1, HcoState%NX
            ...
            phi(I,J) = 0.489_hp - 0.126_hp * f_sand(I,J)
            !=================================================================
            ...
            w(I,J) = rho_w / (rho_p * (1.0_hp - phi(I,J))) * theta(I,J)
            ...
            w_t(I,J) = 0.01_hp * (17.0_hp + 14.0_hp * f_clay(I,J))
  ...
  SUBROUTINE CAL_DUSTL23_EmisFlux(HcoState, ExtState, Inst, DUST_EMIS_FLUX, RC)
    ...
    DO J = 1, HcoState%NY
        DO I = 1, HcoState%NX
            IF (LAI(I,J) > LAI_thr) THEN
                ! calculate f_bare = 1 - LAI / LAI_thr for LAI <= LAI_thr, and f_bare = 0 for LAI > LAI_thr
                f_bare(I,J) = (1.0_hp - Inst%A_water(I,J) - Inst%A_wetland(I,J)) * (1.0_hp - ExtState%FRSNO%Arr%Val(I,J)) * (1.0_hp - LAI(I,J) / LAI_thr)
            ENDIF
        ENDDO
    ENDDO
    ...
    DO J = 1, HcoState%NY
        DO I = 1, HcoState%NX
            IF (Inst%f_clay(I,J) > 0.2_hp) THEN
                f_clay_capped(I,J) = 0.2_hp
            ENDIF
        ENDDO
    ENDDO
    ...
    u_star_t = u_star_it
    DUST_EMIS_FLUX = eta * Inst%SRCE_FUNC * C_tune * C_d * f_bare * f_clay_capped * \
        rho_a * ((u_star_s ** 2.0_hp) - (u_star_t ** 2.0_hp)) / u_star_st * \
        ((u_star_s / u_star_t) ** kappa)
    
    DO J = 1, HcoState%NY
        DO I = 1, HcoState%NX
            IF ((DUST_EMIS_FLUX(I,J) < 0.0_hp) .or. (u_star_s(I,J) .LE. u_star_t(I,J))) THEN
